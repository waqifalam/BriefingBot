 ...
 */
class MichaelWestScrapper {
  /**
   * RSS URL to Scrape
   */
  static RSS_URL = 'https://michaelwest.com.au/feed/'
  ...
  static async getRSSDocument() {
    return await retrieveRSSDocument(MichaelWestScrapper.RSS_URL)
  }
  ...
  static async getBrief() {
    const RSSDocument = await MichaelWestScrapper.getRSSDocument();
    const rssLinks = MichaelWestScrapper.getRSSItemLinks(RSSDocument);
    const linksContentList = await MichaelWestScrapper.getRSSLinksContent(rssLinks);
    return await MichaelWestScrapper.getSummaryForRSSLinksContent(linksContentList);
  }
}

export default MichaelWestScrapper;
